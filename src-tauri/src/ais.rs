...
fn list_in(root: &Path, engine: &str, game: Option<&str>) -> AiList {
    ...
    if engine.trim().is_empty() {
        notes.push(
            "Mashiro does not know the engine version yet, so the engine's own AIs are not listed."
                .into(),
        );
    } else {
    ...
    if ais.is_empty() && notes.is_empty() {
        notes.push("The game archive declares no AIs, so this is Mashiro's built-in list.".into());
    }
    AiList {
        ais,
