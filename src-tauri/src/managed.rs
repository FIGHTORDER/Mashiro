...
/// The directory Mashiro fills when it is managing an install itself.
///
...
///
/// **One directory per game**, named by the game's registry id. It used to be a
/// single `zk` for everything, which was fine while there was one game and
/// wrong the moment somebody could choose: Balanced Annihilation downloaded
/// into a directory called `zk` is then identified as Zero-K and offered
/// Zero-K's lobby and codex.
///
/// Zero-K's id *is* `zk`, so an install made before this keeps its exact path
/// and there is nothing to migrate.
pub fn root_for(app: &tauri::AppHandle, game_id: &str) -> Result<PathBuf, String> {
    let base = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("no data directory: {e}"))?;
    /* The id reaches the filesystem, so it is checked here rather than trusted.
       A discovered game's id carries a `discovered:` prefix and a name that came
       off somebody's disk; neither belongs in a path unexamined. */
    let safe: String = game_id
        .chars()
        .map(|c| if c.is_ascii_alphanumeric() || c == '-' { c } else { '_' })
        .collect();
    if safe.is_empty() {
        return Err("that game has no id to name a directory after".into());
    }
    Ok(base.join(safe))
}

/// The default, for callers with no game in hand.
pub fn root(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    root_for(app, &crate::games::default_game().id)
}

/// Every managed directory that exists, so detection can find whichever game
/// was installed rather than only the default one.
pub fn existing_roots(app: &tauri::AppHandle) -> Vec<PathBuf> {
    crate::games::all()
        .iter()
        .filter_map(|g| root_for(app, &g.id).ok())
        .filter(|d| install::is_managed(d))
        .collect()
}

/// Where a managed install of this game would go, whether or not one is there.
#[tauri::command]
pub fn zks_managed_root(app: tauri::AppHandle, game_id: Option<String>) -> Result<String, String> {
    let id = game_id.unwrap_or_else(|| crate::games::default_game().id);
    Ok(root_for(&app, &id)?.display().to_string())
}
...
pub async fn zks_managed_state(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        managed_state_blocking(app, engine_version, game_id)
    })
    .await
...
#[tauri::command]
pub fn zks_managed_prepare(
    app: tauri::AppHandle,
    game_id: Option<String>,
) -> Result<String, String> {
    let dir = root_for(&app, &game_id.unwrap_or_else(|| crate::games::default_game().id))?;
