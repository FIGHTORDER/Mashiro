...
pub struct Installed {
    ...
    /// Display names of archives that live in `games/`, in the order the cache
    /// listed them.
    ///
    /// Separate from `names` because that folds everything together to answer
    /// "is this here", and a catalogue asking "which games do I have" must not
    /// be answered with a map. The cache records each archive's directory, so
    /// the two are distinguishable without opening anything.
    games: Vec<String>,
...
impl Installed {
    ...
    ///
    /// Names carry versions - `Balanced Annihilation V15.9.8` - because that is
    /// how the engine indexes them and what a start script needs. A caller
    /// matching against a catalogue has to allow for the version itself.
...
fn read_cache(path: &Path, out: &mut Installed) {
    ...
    for entry in text.split("\n\t\t{").skip(1) {
        ...
        /* Which directory it sits in is what makes it a game rather than a map.
           Compared on the last component so a data directory that lives under
           some other path called `games` is not mistaken for the archive kind. */
        if Path::new(&dir)
            .file_name()
            .is_some_and(|n| n.eq_ignore_ascii_case("games"))
            && !out.games.contains(&display)
        {
            out.games.push(display.clone());
        }
        out.insert(&display);
    }
}
...
mod tests {
    ...
    #[test]
    fn a_game_is_told_apart_from_a_map_by_where_it_sits() {
        /* The catalogue asks "which games do I have"; answering with a map
           would put an Installed badge on something nobody installed. The cache
           records each archive's directory, so nothing has to be opened. */
        // Not "kinds": `with_maps` already uses that, both helpers wipe the
        // directory they are given, and cargo runs them at the same time.
        let root = temp("games-vs-maps");
        let games = root.join("games");
        let maps = root.join("maps");
        write(&games, "ba1598.sdz", "x");
        write(&maps, "comet.sd7", "x");
        write(
            &root.join("cache"),
            "ArchiveCache20.lua",
            &format!(
                "archives = {{{}{}}}",
                cache_entry("ba1598.sdz", &games, "Balanced Annihilation V15.9.8"),
                cache_entry("comet.sd7", &maps, "Comet Catcher Redux"),
            ),
        );

        let got = installed(&root);
        assert_eq!(got.games(), ["Balanced Annihilation V15.9.8"]);
        // Both are still present for the "do I have this" question.
        assert!(got.has("Comet Catcher Redux"));
        assert!(got.has("Balanced Annihilation V15.9.8"));
    }
