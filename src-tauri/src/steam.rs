...
//!
//! So `mashiro-steam.exe` carries that dependency instead. If it is missing, or
...
fn helper() -> Option<PathBuf> {
    ...
    let name = if cfg!(windows) { "mashiro-steam.exe" } else { "mashiro-steam" };
...
/// A Steam auth ticket for a game, hex encoded, ready to be a `SteamAuthToken`.
///
/// `game` is a registry id. A ticket is scoped to one Steam application, so
/// signing in to one game with another's ticket does not work - the lobby
/// rejects it, and from the player's side that reads as Steam being broken.
#[tauri::command(async)]
pub fn zks_steam_ticket(app: tauri::AppHandle, game: Option<String>) -> Result<String, String> {
    let Some(path) = helper() else {
        return Err("This copy of Mashiro was built without Steam sign-in.".into());
    };

    let g = game
        .as_deref()
        .and_then(crate::games::by_id)
        .unwrap_or_else(crate::games::default_game);
    let Some(app_id) = g.steam_app_id else {
        return Err(format!("{} is not on Steam, so there is no ticket to get.", g.name));
    };
