//! Thin TCP relay for a lobby server.
//!
...
//!
//! Framing is not here either: where one message stops and the next begins is
//! `transport::LobbyTransport`. This module dials, keeps alive and shuts down,
//! none of which belongs to any one protocol. That split is where a second
//! protocol would actually differ - Tachyon frames over a WebSocket - so what
//! is left below is a socket, and the newline that used to be written into it
...
use crate::transport::LobbyTransport;
...
pub async fn zks_connect(
    ...
    let writer = tokio::spawn(async move {
        let wire = crate::transport::ZkLines;
        while let Some(line) = rx.recv().await {
            if write_half.write_all(&wire.encode(&line)).await.is_err() {
                break;
            }
        }
    });
...
pub async fn zks_send(relay: State<'_, Relay>, line: String) -> Result<(), String> {
    ...
    // The terminator is the transport's business now - see
    // `transport::LobbyTransport::encode`. One definition of where a
    // message ends, rather than one here and another beside the socket.
