...
#[tauri::command]
pub async fn zkw_profile(
    who: String,
    install_root: Option<String>,
) -> Result<Option<Profile>, String> {
    /* The install decides whether this request may leave at all. The nav rail
       hides the Profile screen for a game with no profile service, but the rail
       was the only thing stopping a Beyond All Reason player's name being sent
       to zero-k.info - the command underneath it asked regardless. */
    let Some(service) = crate::services::profiles(install_root.as_deref()) else {
        return Ok(None);
    };
    tauri::async_runtime::spawn_blocking(move || service.profile(&who))
        .await
...
#[tauri::command]
pub async fn zkw_ratings(
    account_id: u32,
    category: u8,
    install_root: Option<String>,
) -> Result<Vec<RatingPoint>, String> {
    // Gated for the same reason as the profile above.
    let Some(service) = crate::services::profiles(install_root.as_deref()) else {
        return Ok(Vec::new());
    };
    tauri::async_runtime::spawn_blocking(move || service.ratings(account_id, category))
        .await
...
/// Zero-K's website as the profile service.
///
/// The scraping above is unchanged; this is the handle the guard hands out, so
/// that a game without a profile service has nothing to be handed.
pub struct ZeroKWeb;

impl crate::services::PlayerProfiles for ZeroKWeb {
    fn profile(&self, who: &str) -> Result<Option<Profile>, String> {
        profile_blocking(who.to_string())
    }

    fn ratings(&self, account_id: u32, category: u8) -> Result<Vec<RatingPoint>, String> {
        ratings_blocking(account_id, category)
    }
}
