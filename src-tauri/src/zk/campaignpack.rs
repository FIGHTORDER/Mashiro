...
pub async fn zks_read_campaign(
    ...
    /* A game with no campaign reads as one with no planets, which is what the
       screen already handles - the nav item hides itself on an empty campaign.
       An error here would put a banner in front of a Beyond All Reason player
       about a feature their game does not have. */
    if !crate::services::has_campaign(install_root.as_deref()) {
        return Ok(serde_json::json!({ "planets": [] }));
    }
