...
use crate::zk::customkey::custom_key;
...
mod tests {
    ...
    fn the_structured_configs_decode_back_to_what_went_in() {
        ...
        let decode = |key: &str| {
            ...
            crate::zk::campaignpack::read_table(
    ...
    fn the_player_gets_what_they_have_unlocked_plus_the_planets_extras() {
        ...
        let list = crate::zk::campaignpack::read_table(
            &format!("return {}", String::from_utf8(bytes).unwrap()),
            "unlocks",
        )
        .unwrap();
    ...
    fn real_scripts_build_for_every_planet() {
        ...
        let source = crate::zk::campaignpack::dir_source(std::path::Path::new(&root));
        let campaign =
            crate::zk::campaignpack::read_campaign_from(source.clone()).expect("the campaign reads");
        let units = crate::zk::campaignpack::read_unit_names(source).expect("the unit roster reads");
