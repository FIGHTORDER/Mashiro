...
use crate::zk::campaignscript::{Commander, Context, Progression};
...
/// `commConfig.lua` - see `crate::zk::campaignpack::read_level_requirements`. Levels start
...
pub async fn zks_galaxy_play(
    ...
    /* Refused rather than answered empty: reading a campaign that is not there
       is a screen with nothing on it, but *starting* a mission is something a
       person asked for and deserves a reason. */
    if !crate::services::has_campaign(install_root.as_deref()) {
        return Err("This game has no campaign for Mashiro to launch.".into());
    }
    let save = load(&app);
    ...
    let bundled = crate::zk::campaignpack::bundled_dir(&app);
    let (campaign, units) = tauri::async_runtime::spawn_blocking(move || {
        ...
        let source = crate::zk::campaignpack::campaign_source(Some(&reading), bundled.as_deref())?;
        let campaign = crate::zk::campaignpack::read_campaign_from(source.clone())?;
        ...
        let units = crate::zk::campaignpack::read_unit_names(source).unwrap_or_else(|why| {
    ...
    let mut script = crate::zk::campaignscript::build(planet_id, planet, &save.progression(), &context)?;
