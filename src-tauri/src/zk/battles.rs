...
const AGENT: &str = "mashiro (third-party Recoil lobby client)";
...
#[tauri::command]
pub async fn zks_search_battles(
    query: BattleQuery,
    install_root: Option<String>,
) -> ArchivePage {
    /* Gated on the install, not just on the rail: the archive is Zero-K's own
       site, and a game without one has nothing here to search. Said plainly
       rather than returned empty, because an empty page reads as "no battles
       matched" and this is not that. */
    let Some(service) = crate::services::battles(install_root.as_deref()) else {
        return ArchivePage {
            note: Some("This game has no battle archive to search.".into()),
            ..Default::default()
        };
    };
    match tauri::async_runtime::spawn_blocking(move || service.search(&query)).await {
        Ok(page) => page,
...
#[tauri::command]
pub async fn zks_lookup_players(
    term: String,
    install_root: Option<String>,
) -> Vec<PlayerMatch> {
    let Some(service) = crate::services::battles(install_root.as_deref()) else {
    ...
    tauri::async_runtime::spawn_blocking(move || service.lookup_players(&term))
        .await
        .unwrap_or_default()
}
...
pub async fn zks_download_replay(
    ...
    let Some(service) = crate::services::battles(install_root.as_deref()) else {
        return Err("This game has no battle archive to download replays from.".into());
    };
    tauri::async_runtime::spawn_blocking(move || {
        let path = service.download_replay(id, install_root.as_deref())?;
...
/// Zero-K's battle archive.
///
/// The scraping above is unchanged; this is the handle the guard hands out, so
/// a game with no archive has nothing to be handed and the request never leaves.
pub struct ZeroKBattles;

impl crate::services::BattleHistory for ZeroKBattles {
    fn search(&self, query: &BattleQuery) -> ArchivePage {
        search(query)
    }

    fn lookup_players(&self, term: &str) -> Vec<PlayerMatch> {
        lookup_players(term)
    }
    ...
    fn download_replay(
        ...
        download_replay(id, install_root)
    }
}
