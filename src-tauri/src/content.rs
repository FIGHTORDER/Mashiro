...
use crate::archives;
use crate::install;
use crate::zk::content as zkcontent;
...
fn try_zk_content(
    ...
    /* The install first, because which content sources apply depends on which
       game is on disk - resolved the same way every other screen resolves it,
       so the download path and the interface cannot disagree about it. */
    let install = match install::detect_with(root) {
        Ok(i) => i,
        Err(e) => return Fallback::Failed(e),
    };
    let game = crate::games::identify(&install.root)
        .unwrap_or_else(crate::games::default_game);

    /* Whichever sources that game has, in order: its own service where it has
       one, then springfiles, which the whole Recoil ecosystem shares. Asking
       Zero-K's service about a Beyond All Reason map was never going to work,
       and its empty answer was indistinguishable from the map not existing. */
    let sources = crate::content_source::chain(&game);
    let mut said = Vec::new();
    let resolved = match crate::content_source::resolve_any(&sources, &item.name, |m| {
        said.push(m)
    }) {
        Some(r) => r,
        None => {
            // A source that failed says so by name; a source that simply did
            // not know stays quiet, which is the ordinary answer for a name
            // belonging to some other game.
            for message in said {
                note(Level::Warn, message);
            }
    ...
    /* Written under whatever capitalisation is already on disk, if any. The
       ...
       archive it already has on every scan. See `zk::content::existing_case`. */
    let dir = install.root.join(resolved.kind.directory());
    let name = crate::zk::content::existing_case(&dir, &name);
    let dest = dir.join(&name);

    note(Level::Info, format!("Not in rapid; looking further afield for {}", item.name));
