...
pub fn make_managed(root: &Path) -> Result<(), String> {
    ...
    if !marker.is_file() {
        std::fs::write(
            &marker,
            concat!(
                "This directory is a game installation managed by Mashiro.\n",
...
/// Register every managed directory that exists.
///
/// There is one per game now, and detection has to be able to find whichever
/// one the player actually installed - registering only the default meant a
/// machine with Balanced Annihilation and no Zero-K looked empty.
pub fn set_managed_roots(dirs: Vec<PathBuf>) {
    if let Some(first) = dirs.first() {
        let _ = MANAGED.set(first.clone());
    }
    if let Ok(mut all) = MANAGED_ALL.lock() {
        *all = dirs;
    }
}

/// Every managed directory, for the probe list.
static MANAGED_ALL: std::sync::Mutex<Vec<PathBuf>> = std::sync::Mutex::new(Vec::new());

fn managed_dirs() -> Vec<PathBuf> {
    MANAGED_ALL.lock().map(|d| d.clone()).unwrap_or_default()
}
...
fn candidates() -> Vec<(PathBuf, String)> {
    let mut out = Vec::new();
    /* Every managed directory first, in registry order, so a machine with more
       than one game installed finds one of them rather than nothing. */
    for dir in managed_dirs() {
        out.push((dir, "Mashiro".to_string()));
    }
    let rest = candidates_with(MANAGED.get().map(PathBuf::as_path));
    for entry in rest {
...
fn candidates_with(managed: Option<&Path>) -> Vec<(PathBuf, String)> {
    ...
    if let Some(dir) = managed {
        out.push((dir.to_path_buf(), "Mashiro".into()));
    }

    /* Every game's own installer directory, in registry order.
     *
     * The names used to be literals - `Programs/Zero-K`, `~/Zero-K` - which is
     * why nothing but Zero-K could ever be found. `games::Game` carries them
     * now, and a game ships with more than one spelling where its installers
     * disagree: BAR appears as `Beyond All Reason`, `Beyond-All-Reason` and
     * `BeyondAllReason` depending on where it came from. */
    let games = crate::games::all();
    if let Ok(local) = std::env::var("LOCALAPPDATA") {
        let programs = PathBuf::from(&local).join("Programs");
        for g in &games {
            for dir in g.dirs_under(&programs) {
                out.push((dir, format!("{} installer", g.name)));
            }
        }
    }
    if let Some(home) = home_dir() {
        for g in &games {
            for dir in g.dirs_under(&home) {
                out.push((dir, "home directory".into()));
            }
        }
        /* Not per game: this is the engine's own data directory, shared by
           every Recoil game on the machine, and a Linux install writes into it
           whichever game it is. */
    ...
    for steam in steam_roots() {
        ...
        for lib in libraries {
            ...
            for g in &games {
                for dir in g.dirs_under(&common) {
                    out.push((dir, "Steam".into()));
                }
            }
        }
    }
...
fn decide_override(path: &Path, found: Option<Install>) -> Result<Install, String> {
    let source = || if is_managed(path) { "Mashiro" } else { "settings" }.to_string();
    if is_filled(path) {
...
mod tests {
    ...
    #[test]
    fn every_registered_game_is_looked_for() {
        /* The whole reason the registry exists. These were literals, so a
           machine with Beyond All Reason and no Zero-K reported no install at
           all - the client could not see a game it fully supports. */
        let probed = candidates_with(None);
        let paths: Vec<String> =
            probed.iter().map(|(p, _)| p.to_string_lossy().to_lowercase()).collect();

        for g in crate::games::all() {
            let found = g.install_dirs.iter().any(|d| {
                let d = d.to_lowercase();
                let tail = format!("/{d}");
                paths.iter().any(|p| p.replace('\\', "/").ends_with(&tail))
            });
            assert!(found, "{} is registered but never looked for: {paths:?}", g.name);
        }
    }

    #[test]
    fn the_engines_own_data_directory_is_not_per_game() {
        /* `~/.config/spring` is shared by every Recoil game on the machine, so
           probing it once per registered game would be the same directory
           reported two or three times over. */
        let probed = candidates_with(None);
        let spring = probed
            .iter()
            .filter(|(p, _)| p.to_string_lossy().replace('\\', "/").ends_with(".config/spring"))
            .count();
        assert!(spring <= 1, "the Spring data directory is probed {spring} times");
    }
    ...
    fn an_empty_managed_directory_does_not_shadow_a_working_install() {
        ...
        let found = choose(&probe(&[(&managed, "Mashiro"), (&steam, "Steam")]))
    ...
    fn a_download_that_stopped_halfway_is_not_an_engine() {
        ...
        let found = choose(&probe(&[(&managed, "Mashiro"), (&steam, "Steam")])).unwrap();
    ...
    fn once_an_engine_lands_the_managed_install_leads_again() {
        ...
        assert_eq!(found.source, "Mashiro");
    ...
    fn an_empty_managed_directory_is_still_found_when_it_is_all_there_is() {
        ...
        let found = choose(&probe(&[(&managed, "Mashiro")])).expect("the marked directory was lost");
    ...
    fn the_install_shiro_made_is_looked_for_first() {
        ...
        assert_eq!(got.first().map(|(_, s)| s.as_str()), Some("Mashiro"));
    }
