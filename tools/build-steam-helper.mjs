 ...
 *
 * - `src-tauri/binaries/mashiro-steam-<triple>.exe`, the helper itself. Tauri
...
const host = triple();
const exeName = `mashiro-steam-${host}${host.includes("windows") ? ".exe" : ""}`;
...
const built = join(HELPER, "target", "release", host.includes("windows") ? "mashiro-steam.exe" : "mashiro-steam");
