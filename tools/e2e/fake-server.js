...
(() => {
  ...
  /* A game the test wants presented instead of the full set. Survives the
     reload that makes the rail re-read it; absent or unparseable means "use the
     full set", so a stale value can never wedge the suite. */
  const readForcedGame = () => {
    try {
      const raw = window.localStorage.getItem("__zks_game");
    ...
    async invoke(cmd, args) {
      switch (cmd) {
        ...
        case "plugin:event|unlisten":
          ...
          return;
        /* Which game the screens present themselves as. The nav rail is gated
           on the services this answers with, so a stub that returned nothing
           would hide the lobby and every test after it would fail on a missing
           button rather than on what it was testing.

           The full set, because the live suite exercises the whole client
           against the fake lobby. `state.game` lets a test narrow it and check
           that a screen really does disappear. */
        case "zks_active_game":
          /* Through localStorage rather than `state`, because the rail is read
             once on load and changing it means reloading - which builds a fresh
             `state` and would throw the override away. */
          return readForcedGame() ?? {
            id: "e2e",
            name: "Recoil",
            installDirs: [],
            archivePrefix: "",
        ...
        /* The rapid catalogue and what is on disk. Two commands rather than
           one because they answer different questions and fail separately: the
           index is a network read, the archive list is a directory read, and a
           machine with no install still has a catalogue to browse.

           The archive names carry versions, as the engine indexes them, so the
           badge test exercises the real matching rather than string equality. */
        case "zks_rapid_games":
          return [
        ...
        case "zks_installed_games":
          /* The third one is the trap, and it is a real pair: the catalogue
             carries both `Tech Annihilation` (techa) and `Tech Annihilation +
             Flea Spam` (tafs). Having only the second must not badge the first,
             which a bare prefix match did. */
          return [
            "Balanced Annihilation V15.9.8",
            "Zero-K v1.14.8.0",
            "Tech Annihilation + Flea Spam v3.8",
          ];

        case "zks_games":
          return [readForcedGame()].filter(Boolean);
        ...
        case "zka_catalogue":
          return [
            { id: "sprofiler", name: "Sprofiler", kind: "executable",
              ...
              source: "github.com/FIGHTORDER/Sprofiler",
            ...
            { id: "splaunch", name: "Splaunch", kind: "executable",
              ...
              source: "github.com/FIGHTORDER/Splaunch",
              unavailable: "No build published yet." },
            { id: "stournament", name: "Stournament", kind: "executable",
              ...
              sha256: "683fd7f9eda333e41d33eb75641e6b5118b989884618c20220c3ec31f392f662" },
            { id: "springen", name: "Springen", kind: "executable",
              ...
              sha256: "2c4756c0f3f961ddc0bb2f56eb88aff9064312055cc94f134feb19916ba9131c" },
          ];
        case "zka_status":
          return [
