 ...
 */
export async function webProfile(
  who: string, installRoot?: string,
): Promise<WebProfile | null> {
  if (!inTauri() || !who.trim()) return null;
  /* The install goes with the request: Rust decides from it whether this game
     has a profile service at all, so that a screen the rail happens to leave
     reachable still cannot send a name to another game's website. */
  return invoke<WebProfile | null>("zkw_profile", { who, installRoot });
}
 ...
 * played. Not an error.
 */
export async function webRatings(
  accountId: number, category = 1, installRoot?: string,
): Promise<RatingPoint[]> {
  if (!inTauri() || !accountId) return [];
  return invoke<RatingPoint[]>("zkw_ratings", { accountId, category, installRoot });
}
