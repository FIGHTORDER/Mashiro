...
export async function searchBattles(
  query: BattleQuery, installRoot?: string,
): Promise<ArchivePage> {
  if (!inTauri()) return OFFLINE;
  try {
    return await invoke<ArchivePage>("zks_search_battles", { query, installRoot });
  } catch (e) {
...
export async function lookupPlayers(
  term: string, installRoot?: string,
): Promise<PlayerMatch[]> {
  if (!inTauri() || !term.trim()) return [];
  try {
    return await invoke<PlayerMatch[]>("zks_lookup_players", { term, installRoot });
  } catch {
    return [];
  }
}
