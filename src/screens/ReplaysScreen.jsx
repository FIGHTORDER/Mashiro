...
export default function Replays({ me, installRoot }) {
  ...
  React.useEffect(() => {
    ...
    let live = true;
    lookupPlayers(me, installRoot).then(found => {
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      lookupPlayers(query, installRoot).then(found => {
        if (!live) return;
  ...
  React.useEffect(() => {
    ...
    return () => { live = false; clearTimeout(timer); };
  }, [source, age, who, mineOnly, myId, installRoot]);

  const loadMore = React.useCallback(() => {
    setLoading(true);
    const players = [who?.id, mineOnly ? myId : undefined].filter(Boolean);
    searchBattles({ age: Number(age) || 0, players: players.length ? players : undefined,
      offset: offset + 40 }, installRoot)
      .then(page => {
