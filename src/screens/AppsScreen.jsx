...
import { installedAlready } from "../net/rapidGames.ts";
        ...
        {catalogue.length === 0 && installed.length === 0 && (
          <div style={{ padding: "var(--sp-6)" }}>
            <EmptyState icon="book-open" title="No campaigns published yet."
              body="Mashiro ships the list of campaigns it knows about, and there are none in it so far. A campaign file you already have will install from the button below." />
          </div>
        )}
      </div>
...
/**
 * Games from the Spring rapid index.
 *
 * The catalogue is Rust's - see `src-tauri/src/rapidrepos.rs` - and installing
 * one goes down the ordinary content path, because a rapid tag is exactly what
 * pr-downloader takes. Nothing here is a second downloader.
 *
 * A game already on disk is marked rather than hidden: "you have this" is a
 * useful thing for a catalogue to say, and hiding it would make the list change
 * shape as things install.
 */
function GamesPanel({ installRoot }) {
  const [state, setState] = React.useState({ loading: true });
  const [busy, setBusy] = React.useState(undefined);
  const [failed, setFailed] = React.useState(undefined);

  /* The catalogue and what is on disk, together: a row cannot say whether it is
     installed until both have answered, and reading them separately made the
     badge appear a beat after the list. */
  const load = React.useCallback(async () => {
    setState({ loading: true });
    try {
      const net = await import("../net/rapidGames.ts");
      const [games, have] = await Promise.all([
        net.rapidGames(), net.installedGames(installRoot),
      ]);
      setState({ games, have });
    } catch (e) {
      setState({ error: String((e && e.message) || e) });
  ...
  const install = async g => {
    setBusy(g.tag);
    setFailed(undefined);
    try {
      const net = await import("../net/content.ts");
      await net.contentFetch("", [{ kind: "game", name: g.tag }], installRoot);
      /* The download runs in Rust and reports on its own stream; re-reading
         what is on disk is how this row learns it arrived. Deliberately after
         the queue call rather than on a timer - the badge is allowed to be a
         download behind, and a poll would be worse than late. */
      const rapid = await import("../net/rapidGames.ts");
      const have = await rapid.installedGames(installRoot);
      setState(prev => ({ ...prev, have }));
    } catch (e) {
      setFailed(String((e && e.message) || e));
    } finally {
      setBusy(undefined);
  ...
  if (state.loading) {
    return <div style={{ padding: "var(--sp-6)", color: "var(--text-faint)" }}>Reading the index…</div>;
  }
  if (state.error) {
    return (
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <EmptyState icon="package" title="The game index could not be read"
          body={state.error}
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, overflowY: "auto" }}>
      {failed && (
        <div style={{ padding: "var(--sp-3) var(--sp-5)", color: "var(--signal-danger)" }}>
          {failed}
        </div>
      )}
      {(state.games || []).map(g => (
        <div key={g.tag} style={{ display: "flex", alignItems: "center", gap: "var(--sp-3)",
          padding: "var(--sp-3) var(--sp-5)", borderBottom: "1px solid var(--w-08)" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ color: "var(--text-hi)" }}>{g.name}</div>
            <div style={{ font: "var(--text-label)", color: "var(--text-faint)" }}>{g.tag}</div>
          </div>
          {installedAlready(g, state.have || [])
            ? <Badge>Installed</Badge>
            : (
              <Button size="sm" variant="secondary" disabled={busy === g.tag}
                onClick={() => install(g)}>
                {busy === g.tag ? "Starting…" : "Install"}
              </Button>
            )}
        </div>
      ))}
...
export default function AppsScreen({ apps = [], statuses = [], onLaunch, onInstall,
  ...
  onCampaigns, activeGame, installRoot }) {
  ...
  /* Widgets, game UI skins and loading screens write into the game's own
     interface, so they are offered only where that interface exists. The
     current kind survives the gate - see `addonKinds.ts`. */
  const kinds = React.useMemo(
    () => visibleKinds(activeGame, kind), [activeGame, kind]);
  const active = KINDS.find(k => k.id === kind) || kinds[0] || KINDS[0];
  ...
  return (
    <div style={{ flex: 1, display: "grid", minHeight: 0,
      ...
      <div style={{ display: "flex", flexDirection: "column", minHeight: 0 }}>
        ...
        {kind === "games" && <GamesPanel installRoot={installRoot} />}
        {!["apps", "skins", "widgets", "uiskins", "campaign", "games"].includes(kind)
          && <NotYet />}
